...
* またtailscale のDNS 設定の項目でMagicDNS 機能をON にしておく必要がある(これがないとデバイス名などをつかったtailnet 経由のアクセスができない. 自分はこれで数時間つまずいた...)
...
```
* `$backend` はservices で指定している` glance` という名前にする
...
```
* nginx はtailscale のネットワークに収めるようにする
...
* また, スマホとかだとtailscale Connect をON にしてるときしかアクセスできないのでConnect 状態にするが,その状だとスマホのときは(PCはあまり思わなかったので違うと思っているが )常にtailscale 経由でネットにアクセスするようになるので全体的にもっさりしました(自分の環境では)
* なのでちょっと使いづらいかもです.(Android ではtailscale を経由しないでダイレクトアクセスする設定などもapp 単位ではできたのですが,ブラウザで特定ページだけとかできるかはわかりません..(しらべられてはない)